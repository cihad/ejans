module CustomFields
  module Target
    ...
    included do
      ## types ##
      %w(belongs_to boolean date has_many image integer place select string tag tree_category).each do |type|
        extend "::CustomFields::Fields::#{type.classify}::ApplyCustomField".constantize
        extend "::CustomFields::Fields::#{type.classify}::ApplyValidate".constantize
      end
      ...
      def build_klass_with_custom_fields(recipe)
        ...
        parent.const_set(name, Class.new(self)).tap do |klass|
          ...
          recipe['rules'].each do |rule|
            ...
            self.send(:"apply_#{rule['type']}_validate", klass, rule)
          end
        end
      end
